...
import { BiometricService, BiometricAvailability } from '../../../core/services/biometric.service';
import { PaymentService, PaymentRequest, PaymentResult } from '../../../core/services/payment.service';
...
export class PaymentConfirmationComponent implements OnInit, OnDestroy {
  ...
  private async checkBiometricAvailability(): Promise<void> {
    ...
    try {
      ...
      this.biometryTypeLabel = this.biometricService.getBiometryTypeLabel(availability.biometryType);
    } catch (error) {
  ...
  async confirmPaymentWithBiometric(): Promise<void> {
    ...
    try {
      ...
      const validation = await this.paymentService.validatePayment(this.paymentRequest);
      if (!validation.valid) {
